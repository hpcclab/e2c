...
class LC(BaseScheduler):
    ...
    def selectMin(self): # form scheduleQ
        #prep process
        mn = config.machines[0]
        min = len(config.machines[0].queue.queue)
        for machine_j in config.machines: # get most free machine
            if (len(machine_j.queue.queue) < min):
                min = len(machine_j.queue.queue)
                mn = machine_j
        return mn
    ...
    def schedule(self):
        ...
        machine = self.selectMin() # getting machine by index
