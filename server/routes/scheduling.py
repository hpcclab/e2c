...
def run_sim():
    ...
    if machine_config:
        ...
        print(
            f"Loading inline config with {len(machine_config.get('machines', []))} machines"
        )
        ...
        results.append(
            {
                ...
                "machineId": task.assigned_machine.id
                if task.assigned_machine
                else None,
                "assigned_machine": machine_info["display_name"]
                if machine_info
                else None,
                "machine_base_name": machine_info["name"] if machine_info else None,
                "machine_replica_number": machine_info["replica_number"]
                if machine_info
                else None,
                ...
                "status": (
                    ...
                    and task.deadline
                    and task.deadline > 0
                    and task.end_time > task.deadline
