  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <Collapsible title="Statistics Summary">
              ...
              <tbody>
                ...
                <tr className="hover:bg-gray-50 bg-yellow-50">
                  ...
                  <td className="border border-gray-300 px-4 py-2 text-right font-semibold text-yellow-700">
                    {(() => {
                      const e = formatEnergy(summaryStats.totalEnergyConsumed);
                      ...
                      {Object.entries(summaryStats.missedByType).map(
                        ([type, count], index) => (
                          ...
                          >
                            ...
                            <td className="border border-gray-300 px-4 py-2 text-right">
                              {(
                                (count / summaryStats.totalMissed) *
                                100
                              ).toFixed(1)}
                              %
                            </td>
                ...
                <tfoot className="bg-yellow-100 font-semibold">
                  <tr>
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-right">
                      {(() => {
                        const e = formatEnergy(
                          summaryStats.totalEnergyConsumed,
                        );
                        return `${e.value} ${e.unit}`;
                      })()}
                    </td>
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart
                  data={machines
                    ...
                    .map((m) => {
                      const kWh =
                        ((m.power || 0) * (m.utilization_time || 0)) / 1000;
                      const e = formatEnergy(kWh);
                      return {
                        name: m.name,
                        energy: parseFloat(e.value),
                        unit: e.unit,
                      };
                    })}
                  margin={{ top: 5, right: 10, left: 0, bottom: 5 }}
                    ...
                    <tr>
                      ...
                      <td className="border border-gray-300 px-4 py-2 text-right">
                        {machines
                          .filter((m) => m.id !== -1)
                          .reduce(
                            (sum, m) => sum + (m.utilization_time || 0),
                            0,
                          )
                          .toFixed(2)}
                      </td>
                      <td className="border border-gray-300 px-4 py-2 text-right text-green-800">
                        ...
                        {machines
                          ...
                          .reduce(
                            (sum, m) =>
                              sum +
                              (m.price || 0) * (m.utilization_time || 0) * 3600,
                            0,
                          )
                          .toFixed(2)}
                      </td>
        ...
        <Collapsible title="Task Results">
          {/* Task Results Table */}
          <div className="mb-6">
