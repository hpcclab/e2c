...
const SimulationReport = ({
  ...
  const summaryStats = useMemo(() => {
    ...
    machineList.forEach(machine => {
      ...
      const count = results.filter(task => {
        ...
        return assignedMachine.startsWith(machine.name + ' #') || 
    ...
    console.log('Tasks per machine (fixed):', tasksPerMachine);
    ...
    ? ((tasksMapped / totalTasks) * 100).toFixed(2) 
    : '0.00';
    ...
    const totalCost = machineList.reduce((sum, m) => 
      sum + ((m.price || 0) * (m.utilization_time || 0)), 0
    );
  ...
  useEffect(() => {
    ...
    console.log('Sample task keys:', dataResults[0] ? Object.keys(dataResults[0]) : 'No tasks');
    console.log('execution_time values:', dataResults.slice(0, 5).map(t => ({
      id: t.id,
            ...
            <div className="overflow-x-auto">
              <table className="table-auto border-collapse border border-gray-300 w-full text-sm shadow-sm">
                <thead className="bg-gradient-to-r from-gray-100 to-gray-200">
                  <tr>
                    <th className="border border-gray-300 px-4 py-2 text-left font-semibold">Machine</th>
                    <th className="border border-gray-300 px-4 py-2 text-right font-semibold">Power (W)</th>
                    <th className="border border-gray-300 px-4 py-2 text-right font-semibold">Price ($/hr)</th>
                    <th className="border border-gray-300 px-4 py-2 text-right font-semibold">Utilization (hr)</th>
                    <th className="border border-gray-300 px-4 py-2 text-right font-semibold">Cost ($)</th>
                    <th className="border border-gray-300 px-4 py-2 text-right font-semibold">Tasks</th>
                  </tr>
                </thead>
                <tbody>
                     {machines
                      ...
                      .map((machine, index) => {
                        ...
                        return (
                          <tr key={`machine-${machine.id}-${index}`} className="hover:bg-gray-50">
                            <td className="border border-gray-300 px-4 py-2 font-medium">{machine.name}</td>
                            <td className="border border-gray-300 px-4 py-2">{machine.power || 0} W</td>
                            <td className="border border-gray-300 px-4 py-2">${machine.price || 0}</td>
                            <td className="border border-gray-300 px-4 py-2">
                              {(machine.utilization_time || 0).toFixed(4)}
                            </td>
                            <td className="border border-gray-300 px-4 py-2">
                              ${((machine.price || 0) * (machine.utilization_time || 0)).toFixed(2)}
                            </td>
                ...
                <tfoot className="bg-gradient-to-r from-gray-200 to-gray-300 font-bold">
                  <tr>
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-right">
                      {machines
                      .filter(m => m.id !== -1)
                      .reduce((sum, m) => sum + (m.power || 0), 0)} W
                    </td>
                    <td className="border border-gray-300 px-4 py-2 text-right">
                      ${machines
                        .filter(m => m.id !== -1)
                    ...
                    {machines
                      .filter(m => m.id !== -1)
                      .reduce((sum, machine) => {
                        const taskCount = summaryStats.tasksPerMachine[machine.id] || 
          ...
          <div className="overflow-x-auto">
            <table className="table-auto border-collapse border border-gray-300 w-full text-sm shadow-sm">
              <thead className="sticky top-0 bg-gradient-to-r from-gray-100 to-gray-200 z-10">
                <tr>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Task ID</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Task Type</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Assigned Machine</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Arrival</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Start</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">End</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Exec Time</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Deadline</th>
                  <th className="border border-gray-300 px-3 py-2 font-semibold">Status</th>
                </tr>
              </thead>
              <tbody>
                {dataResults.map((task, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="border border-gray-300 px-3 py-2">{task.assigned_machine ?? "N/A"}</td>
        ...
        <div className="mt-6 border-t pt-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-red-50 rounded-lg p-4 text-center">
              ...
              <div className="text-sm text-red-600 font-medium">Tasks Missed</div>
            </div>
          </div>
          ...
          <table className="table-auto border-collapse border border-gray-300 w-full text-sm mb-4">
            ...
            <tbody>
              <tr className="hover:bg-gray-50">
                <td className="border border-gray-300 px-4 py-2 font-medium">Total Tasks</td>
                <td className="border border-gray-300 px-4 py-2 text-right">{summaryStats.totalTasks}</td>
              </tr>
              <tr className="hover:bg-gray-50">
                <td className="border border-gray-300 px-4 py-2 font-medium">Tasks Mapped</td>
                <td className="border border-gray-300 px-4 py-2 text-right">{summaryStats.tasksMapped}</td>
              </tr>
              <tr className="hover:bg-gray-50">
              ...
              <tr className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 px-4 py-2 text-right">{summaryStats.totalMissed}</td>
              </tr>
              <tr className="hover:bg-gray-50 bg-blue-50">
                <td className="border border-gray-300 px-4 py-2 font-semibold">Completion Rate</td>
              ...
              <tr className="hover:bg-gray-50 bg-yellow-50">
                <td className="border border-gray-300 px-4 py-2 font-semibold">Total Energy Consumed</td>
          ...
          {summaryStats.missedByType && Object.keys(summaryStats.missedByType).length > 0 && (
            <div className="mt-4">
              <h5 className="text-sm font-semibold text-gray-700 mb-2">Missed Tasks by Type</h5>
              <table className="table-auto border-collapse border border-gray-300 w-full text-sm">
                <thead className="bg-red-50">
                  <tr>
                    ...
                    <th className="border border-gray-300 px-4 py-2 text-right">% of Missed</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.entries(summaryStats.missedByType).map(([type, count], index) => (
                    <tr key={`missed-type-${type}-${index}`} className="hover:bg-gray-50">
          ...
          <div className="mt-4">
            <h5 className="text-sm font-semibold text-gray-700 mb-2">Energy Consumption by Machine</h5>
            <table className="table-auto border-collapse border border-gray-300 w-full text-sm">
              <thead className="bg-yellow-50">
                <tr>
                  <th className="border border-gray-300 px-4 py-2 text-left">Machine</th>
                  <th className="border border-gray-300 px-4 py-2 text-right">Power (W)</th>
                  <th className="border border-gray-300 px-4 py-2 text-right">Utilization (hr)</th>
                  <th className="border border-gray-300 px-4 py-2 text-right">Energy (kWh)</th>
                </tr>
              </thead>
              <tbody>
                {machines
