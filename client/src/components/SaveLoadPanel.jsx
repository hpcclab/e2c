...
import { MdDriveFileRenameOutline, MdDeleteOutline, MdSave, MdSaveAs, MdClose, MdUploadFile, MdDownload } from "react-icons/md";
...
export default function FlowSaveLoadPanel() {
  ...
  const exportWorkspace = () => {
    ...
    triggerDownload(filename, { nodes, edges, machines, iot, taskTypes, scenarioRows, machineConfig });
    notify("Exported!");
  };
    ...
    <>
      ...
      >
        ...
        <div className="modal-body">
          {status && (
            <div className={`status-bar ${status.error ? "status-error" : "status-ok"}`}>
              {status.msg}
            </div>
          )}
              ...
              {files.map((file) => (
                ...
                >
                  <span className="file-name">
                    <VscFiles size={13} style={{ flexShrink: 0, opacity: 0.6 }} />
                    {file}
                  </span>
                  <div style={{ display: "flex", gap: "5px", flexShrink: 0 }}>
                    <button className="file-btn" onClick={(e) => { e.stopPropagation(); load(file); }}>
                      Load
                    </button>
                    <button className="file-btn file-btn-export" onClick={(e) => exportSaved(file, e)} title="Download as file">
                      <MdDownload size={13} />
                    </button>
                  </div>
                </li>
          ...
          {/* Actions */}
          <p className="section-label" style={{ marginTop: "16px" }}>Actions</p>
          <input
          ...
          <button className="action-btn btn-saveas" onClick={saveAs} title="Save as new file">
              <MdSaveAs size={14} /> Save As
            </button>
            <button className="action-btn btn-save" onClick={save} title="Save to selected file">
              <MdSave size={14} /> Overwrite Save
            </button>
            <button className="action-btn btn-rename" onClick={renameFile} title="Rename selected file">
              <MdDriveFileRenameOutline size={14} /> Rename
            </button>
            <button className="action-btn btn-delete" onClick={deleteFile} title="Delete selected file">
              <MdDeleteOutline size={14} /> Delete
            </button>
          </div>

          {/* Export */}
          <p className="section-label" style={{ marginTop: "16px" }}>Export</p>
          <button className="action-btn btn-export" onClick={exportWorkspace} style={{ width: "100%" }}>
            <MdDownload size={14} /> Download workspace as .json
          </button>

          {/* Drop zone */}
          <div className="drop-zone" onDrop={handleDrop} onDragOver={allowDrop} style={{ marginTop: "12px" }}>
